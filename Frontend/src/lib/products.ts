...
export const getProductById = async (productId: string): Promise<Product | null> => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      // Validate that the product has essential fields
      if (product.title && product.category && product.size && product.condition && product.description && product.pointsValue !== undefined) {
        console.log('✅ Valid product found:', product.title);
        return product;
      } else {
        console.warn('⚠️ Product missing essential fields:', product);
        return null;
      }
    } else {
